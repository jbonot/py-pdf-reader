...
class FileReaderApp:
    def __init__(self, root):
        ...
        self.root.geometry("850x275")  # Set the window size to 400 pixels wide and 300 pixels tall
        ...
        self.text_area.grid(row=1, column=0, pady=10, sticky="nsew")  # Stretch to fill space
        ...
        self.button_frame.grid(row=2, column=0, sticky="se")  # Place it below the text area

        # Create a button to copy the text area contents
        self.copy_button = tk.Button(self.button_frame, text="Copy Text", command=self.copy_to_clipboard, state=tk.DISABLED)
        self.copy_button.pack(side=tk.RIGHT, padx=5)  # Align it to the right within the frame
        ...
        self.status_label.grid(row=4, column=0, sticky="se", padx=10, pady=5)  # Bottom right
    ...
    def load_files(self):
        ...
        for index, file_path in enumerate(file_paths):
            if not (file_path.endswith(".pdf")):
                continue
            self.status_label.config(text=f"Processing {index + 1} of {len(file_paths)} files... Please wait.")
