...
class TestUtils:
    ...
    def test_load_config(self):
        ...
        self.assert_test(
            len(self.config.get("windowTitle").strip()) > 0, "value", "TestLoadConfig"
        )
    ...
    def test_get_person_data(self):
        ...
        for index, (input_text, expected_data) in enumerate(cases):
            ...
            if result:
                self.assert_test(
                    result["age"] == expected_data["age"]
                    and result["dob"] == expected_data["dob"]
                    and result["name"] == expected_data["name"],
                    index,
                    "TestGetPersonData",
                )
            else:
    ...
    def test_load_dates(self):
        ...
        for index, entry in enumerate(self.date_entries):
            ...
            self.assert_test(
                entry["day"] == expected_entry["day"]
                and entry["month"] == expected_entry["month"]
                and entry["year"] == expected_entry["year"]
                and entry["name"] == expected_entry["name"],
                index,
                "TestLoadDates",
            )
